...
export class TasksService
{
    ...
    {
        return this._httpClient.get<Task[]>('api/apps/tasks/clear').pipe(
            tap((response) =>
            {
                this._tasks.next([]);
            }),
     ...
     * @param type
     */
    createTask(type: string, params?: any): Observable<Task>
    {
        console.log(arguments);
        return this.tasks$.pipe(
            take(1),
            switchMap(tasks => this._httpClient.post<Task>('api/apps/tasks/task', {type, ...params}).pipe(
                map((newTask) =>
                {
     ...
     * @param type
     */
       createTaskInBulk(params?: any[]): Observable<Task[]>
       {
           console.log(arguments);
           return this.tasks$.pipe(
               take(1),
               switchMap(tasks => this._httpClient.post<Task[]>('api/apps/tasks/task/bulk', params).pipe(
                   map((newTask) =>
                   {
                       // Update the tasks with the new task
