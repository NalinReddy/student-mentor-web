...
export class TasksMockApi
{
    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) =>
            {
                console.log(request.body, "from interecpt");
                const newTask = this.generateTask(request.body);
                ...
                {
                    task.order = task.type === "section" ? 1 : index + 1;
                });
            ...
            this._fuseMockApiService
            .onPost('api/apps/tasks/task/bulk')
            .reply(({request}) =>
            {
                console.log(request.body, "from interecpt");
                if (request.body.length) {
                    request.body.forEach(task => {
                        const newTask = this.generateTask(task);
                        console.log(newTask);
                        // Unshift the new task
                        this._tasks.push(newTask);
                    });
